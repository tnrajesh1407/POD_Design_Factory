...
#   <project>/app/mockup.py   (this file)
#   <project>/assets/tshirt_mockup.jpg
...
def _strip_soft_edge_glow(
    ...
    feather_px: int = 3,
    min_alpha: int = 40,
    brightness_thresh: int = 40,
) -> Image.Image:
    """
    Remove ANY soft glow or drop shadow near alpha edge,
    regardless of color.
    """
    ...
    mask = (a >= min_alpha).astype(np.uint8)
    ...
    # Detect low contrast / glow-like pixels
    # Either very dark or very light near boundary
    glow = band & (gray < brightness_thresh)
...
def _crop_to_alpha(img: Image.Image, pad_pct: float = 0.02) -> Tuple[Image.Image, Tuple[int, int, int, int]]:
    ...
    if pad_pct and pad_pct > 0:
        ...
        # expanded bbox (still refer to original for heuristics)
        return out, bbox

    return cropped, bbox


def _clean_alpha_strong(img: Image.Image,
                        alpha_cutoff: int = 32,
                        blur: float = 0.8,
                        shrink_px: int = 1) -> Image.Image:
    """
    ...
    # Hard cutoff
    a = np.where(a < alpha_cutoff, 0, a)

    # Optional shrink to remove fringe bias
    if shrink_px > 0:
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
        a = cv2.erode(a, kernel, iterations=shrink_px)
...
def _fabric_blend(
    ...
    Parameters
    ----------
    strength : overall fabric influence (0..1)
    detail_strength : how much wrinkle/detail vs broad shading (0..1)
    shading_blur : sigma for broad shading extraction
    ink_bleed : mix-in of base color under print (0..0.2 typical)
    desat : reduce design saturation slightly (0..0.2 typical)
    gamma : optional gamma on modulation (1.0 = none)
    """
    ...
    if shading_blur > 0:
        shading = cv2.GaussianBlur(gray, (0, 0), shading_blur)
    else:
        shading = gray

    # --- wrinkle/detail (high frequency) ---
    # ratio between original and blurred produces wrinkle emphasis around 1.0
    eps = 1e-6
    detail = gray / (shading + eps)
    detail = np.clip(detail, 0.75, 1.25)  # keep sane

    # normalize detail into 0..1
    detail_n = (detail - 0.75) / (1.25 - 0.75)
    detail_n = np.clip(detail_n, 0.0, 1.0)

    # combine broad shading and wrinkles
    mod = (1.0 - detail_strength) * shading + detail_strength * detail_n

    # remap mod to a pleasant range (avoid crushing blacks)
    # 0..1 -> 0.78..1.08
    mod = 0.78 + 0.30 * mod
    ...
    # Apply modulation only where alpha exists
    over_rgb_mod = over_rgb * ((1.0 - strength) + strength * mod3)

    # Slightly desaturate printed ink (helps realism)
    if desat > 0:
        # convert to approx luminance and mix
        lum = (0.114 * over_rgb_mod[:, :, 0] + 0.587 * over_rgb_mod[:, :, 1] + 0.299 * over_rgb_mod[:, :, 2])[:, :, None]
        over_rgb_mod = over_rgb_mod * (1.0 - desat) + lum * desat

    # Ink bleed / fabric pickup: blend a tiny amount of base color under print
    if ink_bleed > 0:
...
def create_mockup(
    ...
    """Create a warped tee mockup.

    - Uses assets/tshirt_mockup.jpg
    - Uses assets/mockup_config.json with quad_norm per mode
    ...
    if not cfg:
        raise RuntimeError(f"Missing config for {mockup_filename}. Create {CONFIG_PATH} with a modes.*.quad_norm entry.")
    ...
    # Load the print-ready image; decide mode based on alpha bbox
    pr = Image.open(print_ready_path).convert("RGBA")

    # OPTIONAL: strip sticker border BEFORE dehalo/crop/warp
    ...
    pr = _clean_alpha_strong(pr, alpha_cutoff=32, blur=0.8, shrink_px=1)

    _, bbox = _crop_to_alpha(pr, pad_pct=0.0)  # bbox only
    ...
    # Optional: auto-center quad to shirt's visual centerline (fixes "slightly off-center" mockups)
    auto_center = False
    ...
    if not _quad_ok(quad, W, H):
        raise RuntimeError(f"Invalid quad for mode '{mode}'. Check mockup_config.json.")

    # Extract design for warping
    preserve_canvas = bool(mode_cfg.get("preserve_canvas", False))

    if preserve_canvas:
        # Keep original print-ready canvas so optical centering stays correct
        design = pr.copy()
    else:
        # For logos/small prints, crop tight
    ...
    # - fallback to cfg.fabric (backward compatible)
    # - mode_cfg.fabric overrides always win
    # -------------------------------
    fabric_cfg: Dict[str, Any] = {}

    # 1) presets (recommended path)
    ...
    # Optional: blend ink with fabric shading/wrinkles for realism
    if fabric_cfg:
    ...
    # Debug outputs (always write even if output is PNG)
    if write_debug:
        area_path, quad_path, warp_path = _debug_paths(output_path)

        # area debug: translucent fill
        ...
        # quad outline: red edges + corner dots
...
if __name__ == "__main__":
    ...
    #   python mockup.py /path/to/04_print_ready.png /path/to/05_mockup.jpg
    import sys

    if len(sys.argv) < 3:
        print("Usage: python mockup.py <print_ready.png> <output_mockup.jpg> [mode]")
        raise SystemExit(1)
    ...
    create_mockup(in_path, out_path, mode=mode_arg, write_debug=True)
